...
#' @param init.loc,init.spread,init.xi sets the initial \eqn{q_\alpha,s_\beta} and \eqn{\xi\in(0,1)} estimates across all dimensions of \code{Y_train}. Overriden by \code{init.wb_path} if \code{!is.null(init.wb_path)}, but otherwise the initial parameters msut be supplied.
...
#' #Linear contribution
#' m_L_2 = 0.3*X_train_lin_s[,,,1]+0.6*X_train_lin_s[,,,2]+0.1*X_train_lin_s[,,,3]-
...
#' #We simulate normal data and estimate the median, i.e., the 50% quantile or mean, as the form for this is known
#'
#' Y=apply(theta,1:3,function(x) rnorm(1,mean=x[1],x[2]))
#'
...
#'
#' #We first use train_quant_NN to estimate the 80% quantile whih will be used as u_train for bGEVPP.NN.train
#' #A simple MLP with no linear or additive components is used
#'
#' model<-train_quant_NN(Y_train, Y_test,X_train=list("X_train_nn"=preds),  type="MLP",
#'            link="identity",tau=0.8,n.ep=1000,
...
#' model2<-bGEVPP.NN.train(Y_train, Y_test,X_train_q,X_train_s, u_train, type="MLP",link.loc="identity",
#'                         n.ep=500, batch.size=50,init.loc=2, init.spread=2,init.xi=0.1,
#'                         widths=c(6,3),seed=1, n_b=12)
#' out2<-bGEVPP.NN.predict(X_train_q=X_train_q,X_train_s=X_train_s,u_train=u_train,model2)
#'
...
#' #To load model, run
#' # model  <- load_model_tf(paste0("model_bGEVPP"),
#' #custom_objects=list("bgev_PP_loss_alpha__beta__p_a__p_b_"=bgev_PP_loss(n_b=12)))
#'
#' #Note that bGEV_PP_loss() can take custom alpha,beta, p_a and p_b arguments if defaults not used
#'
#'
                        ...
                        alpha=0.5,beta=0.5,p_a=0.05,p_b=0.2,n_b=1)
{
  ...
  model %>% compile(
    optimizer="adam",
    loss = bgev_PP_loss(alpha,beta,p_a,p_b,n_b),
    run_eagerly=T
  )
...
bgev_PP_loss <-function(alpha=0.5,beta=0.5,p_a=0.05,p_b=0.2,n_b=1){
