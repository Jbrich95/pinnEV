...
#'
#'@param u.train an array with the same dimension as \code{Y.train}. Gives the threshold above which the bGEV-PP model is fitted, see below. Note that \code{u.train} is applied to both \code{Y.train} and \code{Y.valid}.
#' @param X.train.q  list of arrays corresponding to complementary subsets of the \eqn{d\geq 1} predictors which are used for modelling the location parameter \eqn{q_\alpha}. Must contain at least one of the following three named entries:\describe{
#' \item{\code{X.train.lin.q}}{A 3 or 4 dimensional array of "linear" predictor values. One more dimension than \code{Y.train}. If \code{NULL}, a model without the linear component is built and trained.
...
#' \item{\code{X.train.add.basis.q}}{A 4 or 5 dimensional array of basis function evaluations for the "additive" predictor values.
...
#' @param S_lambda List of smoothing penalty matrices for the splines modelling the effects of \code{X.train.add.basis.q} and \code{X.train.add.basis.s} on their respective parameters; each element only used if \code{!is.null(X.train.add.basis.q)} and \code{!is.null(X.train.add.basis.s)}, respectively. If \code{is.null(S_lambda[[1]])}, then no smoothing penalty used for \code{!is.null(X.train.add.basis.q)}; similarly for the second element and \code{!is.null(X.train.add.basis.s)}. 
...
bGEVPP.NN.train=function(Y.train, Y.valid = NULL,X.train.q,X.train.s, u.train = NULL, type="MLP",link.loc="identity",
                        ...
                        S_lambda=NULL)
{
  ...
  X.train.lin.q=X.train.q$X.train.lin.q
  X.train.add.basis.q=X.train.q$X.train.add.basis.q


  if(!is.null(X.train.nn.q) & !is.null(X.train.add.basis.q) & !is.null(X.train.lin.q) ) {  train.data= list(X.train.lin.q,X.train.add.basis.q,X.train.nn.q); print("Defining lin+GAM+NN model for q_\alpha" );  if(!is.null(Y.valid)) validation.data=list(list(lin_input_q=X.train.lin.q,add_input_q=X.train.add.basis.q,  nn_input_q=X.train.nn.q),Y.valid)}
  if(is.null(X.train.nn.q) & !is.null(X.train.add.basis.q) & !is.null(X.train.lin.q) ) {   train.data= list(X.train.lin.q,X.train.add.basis.q); print("Defining lin+GAM model for q_\alpha" );  if(!is.null(Y.valid)) validation.data=list(list(lin_input_q=X.train.lin.q,add_input_q=X.train.add.basis.q),Y.valid)}
  if(!is.null(X.train.nn.q) & is.null(X.train.add.basis.q) & !is.null(X.train.lin.q) ) { train.data= list(X.train.lin.q,X.train.nn.q); print("Defining lin+NN model for q_\alpha" );  if(!is.null(Y.valid)) validation.data=list(list(lin_input_q=X.train.lin.q, nn_input_q=X.train.nn.q),Y.valid)}
  if(!is.null(X.train.nn.q) & !is.null(X.train.add.basis.q) & is.null(X.train.lin.q) ) {train.data= list(X.train.add.basis.q,X.train.nn.q); print("Defining GAM+NN model for q_\alpha" );  if(!is.null(Y.valid)) validation.data=list(list(add_input_q=X.train.add.basis.q,  nn_input_q=X.train.nn.q),Y.valid)}
  ...
  if(!is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) ) {  train.data= c(train.data,list(X.train.lin.s,X.train.add.basis.s,X.train.nn.s,u.train)); print("Defining lin+GAM+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.train.lin.s,add_input_s=X.train.add.basis.s,  nn_input_s=X.train.nn.s,u_input=u.train)),Y.valid)}
  if(is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) ) {   train.data= c(train.data,list(X.train.lin.s,X.train.add.basis.s,u.train)); print("Defining lin+GAM model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.train.lin.s,add_input_s=X.train.add.basis.s,u_input=u.train)),Y.valid)}
  if(!is.null(X.train.nn.s) & is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) ) { train.data= c(train.data,list(X.train.lin.s,X.train.nn.s,u.train)); print("Defining lin+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.train.lin.s, nn_input_s=X.train.nn.s,u_input=u.train)),Y.valid)}
  if(!is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & is.null(X.train.lin.s) ) {train.data= c(train.data,list(X.train.add.basis.s,X.train.nn.s,u.train)); print("Defining GAM+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(add_input_s=X.train.add.basis.s,  nn_input_s=X.train.nn.s,u_input=u.train)),Y.valid)}
  ...
  if(is.null(X.train.add.basis.s)){S_lambda.s=NULL}

  S_lambda =list("S_lambda.q"=S_lambda.q, "S_lambda.s"=S_lambda.s)
  
  if(type=="CNN" & (!is.null(X.train.nn.q) | !is.null(X.train.nn.s)))print(paste0("Building ",length(widths),"-layer convolutional neural network with ", filter.dim[1]," by ", filter.dim[2]," filter" ))
  if(type=="MLP"  & (!is.null(X.train.nn.q) | !is.null(X.train.nn.s)) ) print(paste0("Building ",length(widths),"-layer densely-connected neural network" ))
  ...
  model<-bGEVPP.NN.build(X.train.nn.q,X.train.lin.q,X.train.add.basis.q,
                         X.train.nn.s,X.train.lin.s,X.train.add.basis.s,
                         u.train,type, init.loc,init.spread,init.xi, widths,filter.dim,link.loc,alpha,beta,p_a,p_b)
...
bGEVPP.NN.predict=function(X.train.q,X.train.s,u.train, model)
{
  ...
  if(!is.null(X.train.nn.q) & !is.null(X.train.add.basis.q) & !is.null(X.train.lin.q) )   train.data= list(X.train.lin.q,X.train.add.basis.q,X.train.nn.q)
  if(is.null(X.train.nn.q) & !is.null(X.train.add.basis.q) & !is.null(X.train.lin.q) )   train.data= list(X.train.lin.q,X.train.add.basis.q)
  if(!is.null(X.train.nn.q) & is.null(X.train.add.basis.q) & !is.null(X.train.lin.q) )  train.data= list(X.train.lin.q,X.train.nn.q)
  if(!is.null(X.train.nn.q) & !is.null(X.train.add.basis.q) & is.null(X.train.lin.q) ) train.data= list(X.train.add.basis.q,X.train.nn.q)
  ...
  if(!is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) )   train.data= c(train.data,list(X.train.lin.s,X.train.add.basis.s,X.train.nn.s,u.train))
  if(is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) )   train.data= c(train.data,list(X.train.lin.s,X.train.add.basis.s,u.train))
  if(!is.null(X.train.nn.s) & is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) )  train.data= c(train.data,list(X.train.lin.s,X.train.nn.s,u.train))
  if(!is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & is.null(X.train.lin.s) ) train.data= c(train.data,list(X.train.add.basis.s,X.train.nn.s,u.train))
  ...
  if(!is.null(X.train.add.basis.q))  gam.weights_q<-matrix(t(model$get_layer("add_q")$get_weights()[[1]]),nrow=dim(X.train.add.basis.q)[length(dim(X.train.add.basis.q))-1],ncol=dim(X.train.add.basis.q)[length(dim(X.train.add.basis.q))],byrow=T)
    if(!is.null(X.train.add.basis.s))  gam.weights_s<-matrix(t(model$get_layer("add_s")$get_weights()[[1]]),nrow=dim(X.train.add.basis.s)[length(dim(X.train.add.basis.s))-1],ncol=dim(X.train.add.basis.s)[length(dim(X.train.add.basis.s))],byrow=T)

    out=list("pred.loc"=pred.loc,"pred.spread"=pred.spread,"pred.xi"=pred.xi)
  if(!is.null(X.train.lin.q) ) out=c(out,list("lin.coeff_q"=c(model$get_layer("lin_q")$get_weights()[[1]])))
  if(!is.null(X.train.lin.s) ) out=c(out,list("lin.coeff_s"=c(model$get_layer("lin_s")$get_weights()[[1]])))
    if(!is.null(X.train.add.basis.q) ) out=c(out,list("gam.weights_q"=gam.weights_q))
    if(!is.null(X.train.add.basis.s) ) out=c(out,list("gam.weights_s"=gam.weights_s))
...
#'
#'
bGEVPP.NN.build=function(X.train.nn.q,X.train.lin.q,X.train.add.basis.q,
                         X.train.nn.s,X.train.lin.s,X.train.add.basis.s,
                         u.train,
                         type, init.loc,init.spread,init.xi, widths,filter.dim,link.loc,alpha,beta,p_a,p_b,c1,c2)
{
  ...
  if(!is.null(X.train.add.basis.s))  input_add_s<- layer_input(shape = dim(X.train.add.basis.s)[-1], name = 'add_input_s')

  #NN input

  if(!is.null(X.train.nn.q))   input_nn_q <- layer_input(shape = dim(X.train.nn.q)[-1], name = 'nn_input_q')
  if(!is.null(X.train.nn.s))   input_nn_s <- layer_input(shape = dim(X.train.nn.s)[-1], name = 'nn_input_s')

  #Linear input

  if(!is.null(X.train.lin.q)) input_lin_q <- layer_input(shape = dim(X.train.lin.q)[-1], name = 'lin_input_q')
  if(!is.null(X.train.lin.s)) input_lin_s <- layer_input(shape = dim(X.train.lin.s)[-1], name = 'lin_input_s')
...
#Spread
  ...
  #Additive towers
  #Location
  n.dim.add_q=length(dim(X.train.add.basis.q))
  ...
  #Spread
  n.dim.add_s=length(dim(X.train.add.basis.s))
  ...
  #Location
  if(!is.null(X.train.lin.q) ) {
    n.dim.lin_q=length(dim(X.train.lin.q))
  ...
  #Spread
  if(!is.null(X.train.lin.s) ) {
    n.dim.lin_s=length(dim(X.train.lin.s))
  ...
  if(!is.null(X.train.add.basis.q) ) input=c(input,input_add_q)
  if(!is.null(X.train.nn.q) ) input=c(input,input_nn_q)
  if(!is.null(X.train.lin.s) ) input=c(input,input_lin_s)
  if(!is.null(X.train.add.basis.s) ) input=c(input,input_add_s)
  if(!is.null(X.train.nn.s) ) input=c(input,input_nn_s)
