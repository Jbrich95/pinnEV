...
bGEV.NN.train=function(Y.train, Y.valid = NULL,X.train.q,X.train.s, type="MLP",link.loc="identity",
                         ...
                         alpha=0.5,beta=0.5,p_a=0.05,p_b=0.2,c1=5,c2=5,S_lambda=NULL)
{
  ...
  if(!is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) ) {  train.data= c(train.data,list(X.train.lin.s,X.train.add.basis.s,X.train.nn.s)); print("Defining lin+GAM+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.train.lin.s,add_input_s=X.train.add.basis.s,  nn_input_s=X.train.nn.s)),Y.valid)}
  if(is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) ) {   train.data= c(train.data,list(X.train.lin.s,X.train.add.basis.s)); print("Defining lin+GAM model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.train.lin.s,add_input_s=X.train.add.basis.s)),Y.valid)}
  if(!is.null(X.train.nn.s) & is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) ) { train.data= c(train.data,list(X.train.lin.s,X.train.nn.s)); print("Defining lin+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.train.lin.s, nn_input_s=X.train.nn.s)),Y.valid)}
  if(!is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & is.null(X.train.lin.s) ) {train.data= c(train.data,list(X.train.add.basis.s,X.train.nn.s)); print("Defining GAM+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(add_input_s=X.train.add.basis.s,  nn_input_s=X.train.nn.s)),Y.valid)}
...
bGEV.NN.predict=function(X.train.q,X.train.s, model)
{
  ...
  if(!is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) )   train.data= c(train.data,list(X.train.lin.s,X.train.add.basis.s,X.train.nn.s))
  if(is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) )   train.data= c(train.data,list(X.train.lin.s,X.train.add.basis.s))
  if(!is.null(X.train.nn.s) & is.null(X.train.add.basis.s) & !is.null(X.train.lin.s) )  train.data= c(train.data,list(X.train.lin.s,X.train.nn.s))
  if(!is.null(X.train.nn.s) & !is.null(X.train.add.basis.s) & is.null(X.train.lin.s) ) train.data= c(train.data,list(X.train.add.basis.s,X.train.nn.s))
