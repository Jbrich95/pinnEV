                         ...
                         alpha=0.5,beta=0.5,p_a=0.05,p_b=0.2,c1=5,c2=5,S_lambda=NULL)
{
  ...
  if(!is.null(X.nn.s) & !is.null(X.add.basis.s) & !is.null(X.lin.s) ) {  train.data= c(train.data,list(X.lin.s,X.add.basis.s,X.nn.s)); print("Defining lin+GAM+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.lin.s,add_input_s=X.add.basis.s,  nn_input_s=X.nn.s)),Y.valid)}
  if(is.null(X.nn.s) & !is.null(X.add.basis.s) & !is.null(X.lin.s) ) {   train.data= c(train.data,list(X.lin.s,X.add.basis.s)); print("Defining lin+GAM model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.lin.s,add_input_s=X.add.basis.s)),Y.valid)}
  if(!is.null(X.nn.s) & is.null(X.add.basis.s) & !is.null(X.lin.s) ) { train.data= c(train.data,list(X.lin.s,X.nn.s)); print("Defining lin+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.lin.s, nn_input_s=X.nn.s)),Y.valid)}
  if(!is.null(X.nn.s) & !is.null(X.add.basis.s) & is.null(X.lin.s) ) {train.data= c(train.data,list(X.add.basis.s,X.nn.s)); print("Defining GAM+NN model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(add_input_s=X.add.basis.s,  nn_input_s=X.nn.s)),Y.valid)}
  if(is.null(X.nn.s) & is.null(X.add.basis.s) & !is.null(X.lin.s) )   {train.data= c(train.data,list(X.lin.s)); print("Defining fully-linear model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(lin_input_s=X.lin.s)),Y.valid)}
  if(is.null(X.nn.s) & !is.null(X.add.basis.s) & is.null(X.lin.s) )   {train.data= c(train.data,list(X.add.basis.s)); print("Defining fully-additive model for s_\beta" );  if(!is.null(Y.valid)) validation.data=list(c(validation.data[[1]],list(add_input_s=X.add.basis.s)),Y.valid)}
...
#' @export
#'
bGEV.NN.predict=function(X.q,X.s, model)
{
  library(tensorflow)
  if(is.null(X.q) &  is.null(X.s)  ) stop("No predictors provided for q_alpha or s_beta: Stationary models are not permitted ")
  ...
  if(!is.null(X.nn.s) & !is.null(X.add.basis.s) & !is.null(X.lin.s) )   train.data= c(train.data,list(X.lin.s,X.add.basis.s,X.nn.s))
  if(is.null(X.nn.s) & !is.null(X.add.basis.s) & !is.null(X.lin.s) )   train.data= c(train.data,list(X.lin.s,X.add.basis.s))
  if(!is.null(X.nn.s) & is.null(X.add.basis.s) & !is.null(X.lin.s) )  train.data= c(train.data,list(X.lin.s,X.nn.s))
  if(!is.null(X.nn.s) & !is.null(X.add.basis.s) & is.null(X.lin.s) ) train.data= c(train.data,list(X.add.basis.s,X.nn.s))
  if(is.null(X.nn.s) & is.null(X.add.basis.s) & !is.null(X.lin.s) )   train.data= c(train.data,list(X.lin.s))
  if(is.null(X.nn.s) & !is.null(X.add.basis.s) & is.null(X.lin.s) )   train.data= c(train.data,list(X.add.basis.s))
  if(!is.null(X.nn.s) & is.null(X.add.basis.s) & is.null(X.lin.s) ) train.data= c(train.data,list(X.nn.s))
  if(is.null(X.nn.s) & is.null(X.add.basis.s) & is.null(X.lin.s) ) train.data= train.data
                       ...
                       type, init.loc,init.spread,init.xi, widths,filter.dim,link.loc,alpha,beta,p_a,p_b,c1,c2)
{
  ...
  if(!is.null(X.nn.q)){
    xiBranch <- input_nn_q %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.nn.q)[-1], trainable=F,
                                           weights=list(matrix(0,nrow=dim(X.nn.q)[length(dim(X.nn.q))],ncol=1),array(1,dim=c(1))), name = 'xi_stationary_dense') %>%
    ...
    xiBranch <- input_nn_s %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.nn.s)[-1], trainable=F,
      ...
      layer_dense(units = 1 ,activation = 'sigmoid',use_bias = F,weights=list(matrix(qlogis(init.xi),nrow=1,ncol=1)), name = 'xi_activation')
  }else  if(!is.null(X.lin.q)){
    xiBranch <- input_lin_q %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.lin.q)[-1], trainable=F,
                                            weights=list(matrix(0,nrow=dim(X.lin.q)[length(dim(X.lin.q))],ncol=1),array(1,dim=c(1))), name = 'xi_stationary_dense') %>%
      layer_dense(units = 1 ,activation = 'sigmoid',use_bias = F,weights=list(matrix(qlogis(init.xi),nrow=1,ncol=1)), name = 'xi_activation')
    ...
    xiBranch <- input_lin_s %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.lin.s)[-1], trainable=F,
      ...
      layer_dense(units = 1 ,activation = 'sigmoid',use_bias = F,weights=list(matrix(qlogis(init.xi),nrow=1,ncol=1)), name = 'xi_activation')
  }else if(!is.null(X.add.basis.q)){
    xiBranch <- input_add_q %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.add.basis.q)[-1], trainable=F,
                                            weights=list(matrix(0,nrow=dim(X.add.basis.q)[length(dim(X.add.basis.q))],ncol=1),array(1,dim=c(1))), name = 'xi_stationary_dense') %>%
      layer_dense(units = 1 ,activation = 'sigmoid',use_bias = F,weights=list(matrix(qlogis(init.xi),nrow=1,ncol=1)), name = 'xi_activation')
  ...
  if(is.null(X.nn.q) & is.null(X.add.basis.q) & is.null(X.lin.q)) {
    ...
    if(!is.null(X.nn.s)){
      statBranchq <- input_nn_s %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.nn.s)[-1], trainable=F,
                                                weights=list(matrix(0,nrow=dim(X.nn.s)[length(dim(X.nn.s))],ncol=1),array(1,dim=c(1))), name = 'q_stationary_dense1') %>%
      ...
      statBranchq <- input_lin_s %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.lin.s)[-1], trainable=F,
                                                 weights=list(matrix(0,nrow=dim(X.lin.s)[length(dim(X.lin.s))],ncol=1),array(1,dim=c(1))), name = 'q_stationary_dense1') %>%
        layer_dense(units = 1 ,activation = 'linear',use_bias = F,weights=list(matrix(array(init.loc),nrow=1,ncol=1)), name = 'q_stationary_dense2')
    }else  if(!is.null(X.add.basis.s)){
      statBranchq <- input_add_s %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.add.basis.s)[-1], trainable=F,
                                                 weights=list(matrix(0,nrow=dim(X.add.basis.s)[length(dim(X.add.basis.s))],ncol=1),array(1,dim=c(1))), name = 'q_stationary_dense1') %>%
        layer_dense(units = 1 ,activation = 'linear',use_bias = F,weights=list(matrix(array(init.loc),nrow=1,ncol=1)), name = 'q_stationary_dense2')
    }
  ...
  if(is.null(X.nn.s) & is.null(X.add.basis.s) & is.null(X.lin.s)) {
    ...
    if(!is.null(X.nn.q)){
      statBranchs <- input_nn_q %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.nn.q)[-1], trainable=F,
                                                weights=list(matrix(0,nrow=dim(X.nn.q)[length(dim(X.nn.q))],ncol=1),array(1,dim=c(1))), name = 's_stationary_dense1') %>%
      ...
      statBranchs <- input_lin_q %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.nn.q)[-1], trainable=F,
                                                 weights=list(matrix(0,nrow=dim(X.nn.q)[length(dim(X.nn.q))],ncol=1),array(1,dim=c(1))), name = 's_stationary_dense1') %>%
        layer_dense(units = 1 ,activation = 'linear',use_bias = F,weights=list(matrix(array(init.spread),nrow=1,ncol=1)), name = 's_stationary_dense2')
    }else  if(!is.null(X.add.basis.q)){
      statBranchs <- input_add_q %>% layer_dense(units = 1 ,activation = 'relu', input_shape =dim(X.nn.q)[-1], trainable=F,
                                                 weights=list(matrix(0,nrow=dim(X.nn.q)[length(dim(X.nn.q))],ncol=1),array(1,dim=c(1))), name = 's_stationary_dense1') %>%
        layer_dense(units = 1 ,activation = 'linear',use_bias = F,weights=list(matrix(array(init.spread),nrow=1,ncol=1)), name = 's_stationary_dense2')
    }
