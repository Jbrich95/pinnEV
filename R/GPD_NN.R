...
#'
#'@param u_train an array with the same dimension as \code{Y_train}. Gives the threshold used to create exceedances of \code{Y_train}, see below. Note that \code{u_train} is applied to both \code{Y_train} and \code{Y_valid}.
...
#' @param init.scale,init.xi sets the initial \eqn{sigma} and \eqn{\xi\in(0,1)} estimates across all dimensions of \code{Y_train}. Overridden by \code{init.wb_path} if \code{!is.null(init.wb_path)}, but otherwise the initial parameters must be supplied.
...
#' @export
  ...
  GPD.NN.train=function(Y_train, Y_valid = NULL,X_train,u_train = NULL,   type="MLP",link="identity",tau=NULL,
                          n.ep=100, batch.size=100,init.scale=NULL,init.xi=NULL, widths=c(6,3), filter.dim=c(3,3),seed=NULL,init.wb_path=NULL)
  {
    
  
  
  
  if(is.null(X_train)  ) stop("No predictors provided for sigma")
  ...
  if(!is.null(X_train_nn) & !is.null(X_train_add_basis) & !is.null(X_train_lin) ) {  train.data= list(X_train_lin,X_train_add_basis,X_train_nn,u_train); print("Defining lin+GAM+NN model for sigma" );  if(!is.null(Y_valid)) validation.data=list(list(lin_input_sigma=X_train_lin,add_input_sigma=X_train_add_basis,  nn_input_sigma=X_train_nn,u_input=u_train),Y_valid)}
  if(is.null(X_train_nn) & !is.null(X_train_add_basis) & !is.null(X_train_lin) ) {   train.data= list(X_train_lin,X_train_add_basis,u_train); print("Defining lin+GAM model for sigma" );  if(!is.null(Y_valid)) validation.data=list(list(lin_input_sigma=X_train_lin,add_input_sigma=X_train_add_basis,u_input=u_train),Y_valid)}
  if(!is.null(X_train_nn) & is.null(X_train_add_basis) & !is.null(X_train_lin) ) { train.data= list(X_train_lin,X_train_nn,u_train); print("Defining lin+NN model for sigma" );  if(!is.null(Y_valid)) validation.data=list(list(lin_input_sigma=X_train_lin, nn_input_sigma=X_train_nn,u_input=u_train),Y_valid)}
  if(!is.null(X_train_nn) & !is.null(X_train_add_basis) & is.null(X_train_lin) ) {train.data= list(X_train_add_basis,X_train_nn,u_train); print("Defining GAM+NN model for sigma" );  if(!is.null(Y_valid)) validation.data=list(list(add_input_sigma=X_train_add_basis,  nn_input_sigma=X_train_nn,u_input=u_train),Y_valid)}
  if(is.null(X_train_nn) & is.null(X_train_add_basis) & !is.null(X_train_lin) )   {train.data= list(X_train_lin,u_train); print("Defining fully-linear model for sigma" );  if(!is.null(Y_valid)) validation.data=list(list(lin_input_sigma=X_train_lin,u_input=u_train),Y_valid)}
  if(is.null(X_train_nn) & !is.null(X_train_add_basis) & is.null(X_train_lin) )   {train.data= list(X_train_add_basis,u_train); print("Defining fully-additive model for sigma" );  if(!is.null(Y_valid)) validation.data=list(list(add_input_sigma=X_train_add_basis,u_input=u_train),Y_valid)}
  if(!is.null(X_train_nn) & is.null(X_train_add_basis) & is.null(X_train_lin) )   {train.data= list(X_train_nn,u_train); print("Defining fully-NN model for sigma" );  if(!is.null(Y_valid)) validation.data=list(list( nn_input_sigma=X_train_nn,u_input=u_train),Y_valid)}
  ...
  model<-GPD.NN.build(X_train_nn,X_train_lin,X_train_add_basis,
                         u_train,type,init.scale,init.xi, widths,filter.dim)
  ...
  if(!is.null(Y_valid)) checkpoint <- callback_model_checkpoint(paste0("model_GPD_checkpoint"), monitor = "val_loss", verbose = 0,   save_best_only = TRUE, save_weights_only = TRUE, mode = "min",   save_freq = "epoch") else checkpoint <- callback_model_checkpoint(paste0("model_GPD_checkpoint"), monitor = "loss", verbose = 0,   save_best_only = TRUE, save_weights_only = TRUE, mode = "min",   save_freq = "epoch")
...
#' @export
#'
  GPD.NN.predict=function(X_train,u_train, model)
{
  library(tensorflow)
  if(is.null(X_train)  ) stop("No predictors provided for sigma")
  ...
  if(!is.null(X_train_nn) & !is.null(X_train_add_basis) & !is.null(X_train_lin) )   train.data= list(X_train_lin,X_train_add_basis,X_train_nn,u_train)
  if(is.null(X_train_nn) & !is.null(X_train_add_basis) & !is.null(X_train_lin) )   train.data= list(X_train_lin,X_train_add_basis,u_train)
  if(!is.null(X_train_nn) & is.null(X_train_add_basis) & !is.null(X_train_lin) )  train.data= list(X_train_lin,X_train_nn,u_train)
  if(!is.null(X_train_nn) & !is.null(X_train_add_basis) & is.null(X_train_lin) ) train.data= list(X_train_add_basis,X_train_nn,u_train)
  if(is.null(X_train_nn) & is.null(X_train_add_basis) & !is.null(X_train_lin) )   train.data= list(X_train_lin,u_train)
  if(is.null(X_train_nn) & !is.null(X_train_add_basis) & is.null(X_train_lin) )   train.data= list(X_train_add_basis,u_train)
  if(!is.null(X_train_nn) & is.null(X_train_add_basis) & is.null(X_train_lin) )   train.data= list(X_train_nn,u_train)
  ...
  if(!is.null(X_train_add_basis))  gam.weights_sigma<-matrix(t(model$get_layer("add_sigma")$get_weights()[[1]]),nrow=dim(X_train_add_basis)[length(dim(X_train_add_basis))-1],ncol=dim(X_train_add_basis)[length(dim(X_train_add_basis))],byrow=T)

  out=list("pred.sigma"=pred.sigma,"pred.xi"=pred.xi)
  if(!is.null(X_train_lin) ) out=c(out,list("lin.coeff_sigma"=c(model$get_layer("lin_sigma")$get_weights()[[1]])))
  if(!is.null(X_train_add_basis) ) out=c(out,list("gam.weights_sigma"=gam.weights_sigma))
...
#'
#'
GPD.NN.build=function(X_train_nn,X_train_lin,X_train_add_basis,
                         u_train,
                         type,init.scale,init.xi, widths,filter.dim)
{
  #Additive input
  if(!is.null(X_train_add_basis))  input_add_sigma<- layer_input(shape = dim(X_train_add_basis)[-1], name = 'add_input_sigma')
  
  #NN input
  
  if(!is.null(X_train_nn))   input_nn_sigma <- layer_input(shape = dim(X_train_nn)[-1], name = 'nn_input_sigma')
  
  #Linear input
  if(!is.null(X_train_lin)) input_lin_sigma <- layer_input(shape = dim(X_train_lin)[-1], name = 'lin_input_sigma')
    ...
    if(type=="MLP"){
      for(i in 1:n.layers){
        nnBranchsigma <- nnBranchsigma  %>% layer_dense(units=nunits[i],activation = 'relu',
                                                input_shape =dim(X_train_nn)[-1], name = paste0('nn_sigma_dense',i) )
      }
    }else if(type=="CNN"){
      for(i in 1:n.layers){
        nnBranchsigma <- nnBranchsigma  %>% layer_conv_2d(filters=nunits[i],activation = 'relu',kernel_size=c(filter.dim[1],filter.dim[2]), padding='same',
                                                  input_shape =dim(X_train_nn)[-1], name = paste0('nn_sigma_cnn',i) )
      }
  ...
  #Scale
  n.dim.add_sigma=length(dim(X_train_add_basis))
  if(!is.null(X_train_add_basis) & !is.null(X_train_add_basis) ) {
    
    addBranchsigma <- input_add_sigma %>%
      layer_reshape(target_shape=c(dim(X_train_add_basis)[2:(n.dim.add_sigma-2)],prod(dim(X_train_add_basis)[(n.dim.add_sigma-1):n.dim.add_sigma]))) %>%
      layer_dense(units = 1, activation = 'linear', name = 'add_sigma',
                  weights=list(matrix(0,nrow=prod(dim(X_train_add_basis)[(n.dim.add_sigma-1):n.dim.add_sigma]),ncol=1)),use_bias = F)
  }
  if(!is.null(X_train_add_basis) & is.null(X_train_add_basis) ) {
    ...
    addBranchsigma <- input_add_sigma %>%
      layer_reshape(target_shape=c(dim(X_train_add_basis)[2:(n.dim.add_sigma-2)],prod(dim(X_train_add_basis)[(n.dim.add_sigma-1):n.dim.add_sigma]))) %>%
      layer_dense(units = 1, activation = 'linear', name = 'add_sigma',
                  weights=list(matrix(0,nrow=prod(dim(X_train_add_basis)[(n.dim.add_sigma-1):n.dim.add_sigma]),ncol=1),array(init.scale)),use_bias = T)
  }
  #Linear towers
  ...
  if(!is.null(X_train_lin) ) {
    n.dim.lin_sigma =length(dim(X_train_lin))
    
    if(is.null(X_train_nn) & is.null(X_train_add_basis )){
      linBranchsigma <- input_lin_sigma%>%
        layer_dense(units = 1, activation = 'linear',
                    input_shape =dim(X_train_lin)[-1], name = 'lin_sigma',
                    weights=list(matrix(0,nrow=dim(X_train_lin)[n.dim.lin_sigma],ncol=1),array(init.scale)),use_bias=T)
    }else{
      linBranchsigma <- input_lin_sigma%>%
        layer_dense(units = 1, activation = 'linear',
                    input_shape =dim(X_train_lin)[-1], name = 'lin_sigma',
                    weights=list(matrix(0,nrow=dim(X_train_lin)[n.dim.lin_sigma],ncol=1)),use_bias=F)
    }
  }
  
 
  #Scale
  if(!is.null(X_train_nn) & !is.null(X_train_add_basis) & !is.null(X_train_lin) )  sigmaBranchjoined <- layer_add(inputs=c(addBranchsigma,  linBranchsigma,nnBranchsigma))  #Add all towers
  if(is.null(X_train_nn) & !is.null(X_train_add_basis) & !is.null(X_train_lin) )  sigmaBranchjoined <- layer_add(inputs=c(addBranchsigma,  linBranchsigma))  #Add GAM+lin towers
  if(!is.null(X_train_nn) & is.null(X_train_add_basis) & !is.null(X_train_lin) )  sigmaBranchjoined <- layer_add(inputs=c(  linBranchsigma,nnBranchsigma))  #Add nn+lin towers
  if(!is.null(X_train_nn) & !is.null(X_train_add_basis) & is.null(X_train_lin) )  sigmaBranchjoined <- layer_add(inputs=c(addBranchsigma,  nnBranchsigma))  #Add nn+GAM towers
  if(is.null(X_train_nn) & is.null(X_train_add_basis) & !is.null(X_train_lin) )  sigmaBranchjoined <- linBranchsigma  #Just lin tower
  if(is.null(X_train_nn) & !is.null(X_train_add_basis) & is.null(X_train_lin) )  sigmaBranchjoined <- addBranchsigma  #Just GAM tower
  if(!is.null(X_train_nn) & is.null(X_train_add_basis) & is.null(X_train_lin) )  sigmaBranchjoined <- nnBranchsigma  #Just nn tower
  ...
  if(!is.null(X_train_lin) ) input=c(input,input_lin_sigma)
  if(!is.null(X_train_add_basis) ) input=c(input,input_add_sigma)
  if(!is.null(X_train_nn) ) input=c(input,input_nn_sigma)
